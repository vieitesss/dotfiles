...
# Zerobrew: expose linked packages and versioned/keg-only formulae (e.g. zig@0.15)
if [[ -z "${ZEROBREW_ROOT:-}" ]]; then
    if [[ -d "/opt/zerobrew" || $IS_DARWIN -eq 1 ]]; then
...
if [[ -z "${ZEROBREW_PREFIX:-}" ]]; then
    if [[ $IS_DARWIN -eq 1 ]]; then
        export ZEROBREW_PREFIX="$ZEROBREW_ROOT"
    else
        export ZEROBREW_PREFIX="$ZEROBREW_ROOT/prefix"
    fi
fi

if [[ -d "$ZEROBREW_PREFIX" ]]; then
    path=("$ZEROBREW_PREFIX/bin" "$ZEROBREW_PREFIX"/opt/*/bin(N-/) $path)
fi
